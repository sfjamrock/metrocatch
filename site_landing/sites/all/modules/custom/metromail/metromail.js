  ...
  Drupal.behaviors.metroMailMore = function(context) {
    ...
    $(settings.element, context).click(function() {
      ...
      settings.callbackURL = Drupal.settings.metromail.base_url + $(this).attr('href').split('#')[1];
  ...
  Drupal.behaviors.metroMailThread = function(context) {
    ...
    $(settings.element, context).mousedown(function(event) {
      ...
      settings.element = $(this);
      var thread_id = settings.element.attr('id').split('expand-')[1];
    ...
    if (context == document && typeof(Drupal.settings.metromail.scroll_to) != 'undefined') {
